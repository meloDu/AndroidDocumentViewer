...
import static com.hanlyjiang.library.fileviewer.demos.FileViewApplication.FILE_DIR;
...
//            });
    ...
    private void initView() {
        findViewById(R.id.btn_open_pdf_with_mupdf).setOnClickListener(view -> {
            String fileName = getFilePath("01100190041147682910.pdf");
            startMuPDFActivityWithExampleFile(getFilePath("北大公学企业宣传册－终稿.pdf"));
//            startMuPDFActivityWithExampleFile(getFilePath("绩效评估表-杜彦峰10-12.xlsx"));
        });



        findViewById(R.id.btn_open_pdf_with_tbs).setOnClickListener((view) -> {
            openFileWithTbs(getFilePath("啊啊啊啊啊啊.pdf"));
        });
        findViewById(R.id.btn_open_doc_with_tbs).setOnClickListener(view -> openFileWithTbs(getFilePath("新建 Microsoft Word 文档.docx")));
        findViewById(R.id.btn_open_ppt_with_tbs).setOnClickListener(view -> openFileWithTbs(getFilePath("TestPPT.ppt")));
        findViewById(R.id.btn_open_excel_with_tbs).setOnClickListener(view -> openFileWithTbs(getFilePath("绩效评估表-杜彦峰10-12.xlsx")));
    }
